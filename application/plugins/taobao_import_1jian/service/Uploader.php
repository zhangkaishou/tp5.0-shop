...
class Uploader
{
    ...
    private function saveRemote()
    {
        ...
        if (!(stristr($heads[0], "200") && stristr($heads[0], "OK")) && !(stristr($heads[0], "302")) && strstr($imgUrl,".mp4")) {
 ...
 //print_r($heads);
  ...
  $img=null;
  //echo "上传视频原始链接:".$imgUrl."\n";
if(strstr($imgUrl,"mp4"))
{
    ...
    ob_end_clean();
        ...
        $this->filePath = $this->getFilePath();
        $this->fileName = $this->getFileName();
        //echo "上传的文件名是".$this->fullName."\n";
        $dirname = dirname($this->filePath);

        //检查文件大小是否超出限制
        if (!$this->checkSize() && 1==2) {
            $this->stateInfo = $this->getStateInfo("ERROR_SIZE_EXCEED");
            return;
        }
